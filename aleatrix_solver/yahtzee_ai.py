...
class YahtzeeAI:
    ...
    def evaluate_action_ev(self, open_categories, current_dice, rolls_left, upper_sum, yahtzee_scored, action_type, target_idx, risk_level=0.0):
        """
        Calculates the exact expected value (EV) for a specific action from the current state.
        - action_type: 0 for 'score', 1 for 'keep'
        - target_idx: category index (0-12) if score, keep mask (0-31) if keep
        """
        def keep_values_from_mask(sorted_dice, target_mask):
        ...
        if action_type == 0:
            ...
            if self._should_use_exact_endgame(open_categories):
                category_mask = self._category_mask(open_categories)
                risk_key = self._risk_key(risk_level)
                dice_index = DICE_INDEX[sorted_dice]
                cost_factor = ((len(open_categories) - 1) / 12.0) ** self.decay_exponent if len(open_categories) > 1 else 0.0

                cat_index = target_idx
                points, new_upper_sum, new_yahtzee_scored = self._score_transition_index(
                    cat_index,
                    dice_index,
                    upper_sum,
                ...
                upper_cats = UPPER_CATEGORIES
                open_upper_cats = [c for c in open_categories if c in upper_cats]

                if cat in upper_cats:
                    new_mu = sum(UPPER_STATS[c]['mu'] for c in open_upper_cats if c != cat)
                    new_var = sum(UPPER_STATS[c]['var'] for c in open_upper_cats if c != cat)
                    new_max = sum(MAX_POSSIBLE_CAT[c] for c in open_upper_cats if c != cat)
                    bonus_utility = self.bonus_multiplier * get_bonus_prob_opt(upper_sum + score, new_mu, new_var, new_max)
                else:
                    total_mu = sum(UPPER_STATS[c]['mu'] for c in open_upper_cats)
                    total_var = sum(UPPER_STATS[c]['var'] for c in open_upper_cats)
                    total_max = sum(MAX_POSSIBLE_CAT[c] for c in open_upper_cats)
                    bonus_utility = self.bonus_multiplier * get_bonus_prob_opt(upper_sum, total_mu, total_var, total_max)
            ...
            if rolls_left <= 0:
                return -999999.0
            if not (0 <= target_idx < 32):
                return -999999.0

            keep = tuple(keep_values_from_mask(sorted_dice, target_idx))
            ...
            if self._should_use_exact_endgame(open_categories):
                ...
                # Heuristic keeping
                num_to_roll = 5 - len(keep)
                cost_factor = ((len(open_categories) - 1) / 12.0) ** self.decay_exponent if len(open_categories) > 1 else 0.0
                baseline_cost = {c: self.baselines[c] * cost_factor for c in open_categories}
                risk_adjust = {c: self._risk_adjustment(c, risk_level, cost_factor) for c in open_categories}
                cache_r = self.cache[yahtzee_scored][rolls_left - 1]
                ...
                bonus_utility_by_dice = {}
                for c in open_upper_cats:
                    mu_val = total_mu - UPPER_STATS[c]['mu']
                    var_val = total_var - UPPER_STATS[c]['var']
                    max_val = total_max - MAX_POSSIBLE_CAT[c]
                    cat_cache = cache_r[c]
                    bonus_utility_by_dice[c] = [
                        self.bonus_multiplier * get_bonus_prob_opt(upper_sum + expected_s_val, mu_val, var_val, max_val)
                        for expected_s_val in cat_cache
                    ]
                ...
                for roll, prob in outcomes:
                    ...
                    for c in open_categories:
                        expected_s = cache_r[c][final_dice_index]
                        cost_val = baseline_cost[c]

                        if c in upper_cats:
                            bonus_utility = bonus_utility_by_dice[c][final_dice_index]
                        else:
                            bonus_utility = current_bonus_utility

                        extra_yahtzee_bonus = 100 if (is_y_scored and c != 'yahtzee') else 0
